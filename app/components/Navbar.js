...
export default function Navbar({ cartCount, onEnquireClick, onCartClick, onHome, onProducts }) {
  ...
  const sidebar = (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      {/* Sidebar */}
      <div className="relative bg-[#18181b]/95 w-64 h-[calc(100vh-5rem)] mt-20 shadow-2xl p-8 flex flex-col gap-8 items-center justify-center rounded-l-2xl animate-fade-in-down border-l border-gray-800">
        <button className="absolute top-4 right-4 text-gray-400 hover:text-gray-100 text-2xl" onClick={() => setSidebarOpen(false)}>&times;</button>
  ...
  return (
    <nav className="w-full fixed left-0 top-0 z-30 bg-white/80 dark:bg-black/70 backdrop-blur shadow flex items-center justify-between px-6 py-3">
      ...
      <button className="sm:hidden flex items-center text-3xl text-gray-900 dark:text-gray-100" onClick={() => setSidebarOpen(true)} aria-label="Open menu">
